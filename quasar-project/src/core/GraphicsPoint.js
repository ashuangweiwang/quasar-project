...
class GraphicsPoint extends PIXI.Graphics {
	constructor(editTool) {
		...
		this.icon='img:images\\point.png'
		//this.disabled=true
	...
	CalHitArea(points) {
		...
		if (this.hitArea == null)
			this.hitArea = new PIXI.Polygon(hitPoly);
	}
	...
	DrawGraphic() {
		...
		for (var i = 0; i < this.GraphicData.length; i++) {
			...
			if (this.GraphicData[i].GeoType == 'Line') {
				...
				if (linestyle != null)
					this.lineStyle(linestyle.width, linestyle.color, linestyle.alpha, 0.5, false);

				var hitPoly = [];
				this.moveTo(this.GraphicData[i].points[0].x, this.GraphicData[i].points[0].y);
				// if(0)
				for (var j = 0; j < this.GraphicData[i].points.length - 2; j++) {
					...
					for (var t = 0; t <= N; t++) {
						...
						if (this.Smooth == 0) {
							...
							p = Hermit(pos, pos1, pos2, t * 1.000 / N);
						}
				...
				if (this.fillStyle == null) {
					...
					this.fillStyle.color = PIXI.utils.rgb2hex([Math.random() + 0.1, Math.random() + 0.1, Math.random() + 0.1]);;
			...
			else if(this.GraphicData[i].GeoType == 'Point')
			{
				...
				for (var j= 0; j < this.GraphicData[i].points.length; j++) {
                    ...
                    if(this.Style==null){
                        ...
                        this.Style.color=PIXI.utils.rgb2hex([Math.random() + 0.1, Math.random() + 0.1, Math.random() + 0.1]);
                        this.Style.width=3
                    }
				this.beginFill(this.Style.color);
				this.drawCircle(this.GraphicData[i].points[j].x,this.GraphicData[i].points[j].y, this.Style.width)
				this.endFill();
	...
	onDragStart(event) {
		...
		this.alpha = 0.5;
		this.dragging = true;
		this.EditTools.EditOnePoint(this);///问题应该在这？？？找不到edittool无法编辑
